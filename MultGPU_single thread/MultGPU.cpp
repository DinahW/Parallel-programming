...
void runJacobi1DCUDA(int tsteps, int n, double* A, double* B, double* A_outputFromGpu, double* B_outputFromGpu) {
  ...
  for (int t = 0; t < tsteps ; ++t) {
    ...
    //FIXME/FIXED
    cudaMemcpyPeer(&d_A0[n/2], 0, &d_A1[0], 1, sizeof(double));
    cudaMemcpyPeer(&d_A1[0], 1, &d_A0[n/2], 0, sizeof(double));
    // ---> update halos directly using cudaMemcpy transfers version 2 <--- //
    //FIXME/FIXED
	cudaMemcpy(&d_A0[n/2], &d_A1[0], sizeof(double), cudaMemcpyDeviceToDevice);
    cudaMemcpy(&d_A1[0], &d_A0[n/2], sizeof(double), cudaMemcpyDeviceToDevice);

    // ---> update through host version 3<--- //
    //FIXME/FIXED
    double tmp;
    cudaMemcpy(&tmp, &d_A1[0], sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&d_A0[n/2], &tmp, sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(&tmp, &d_A0[n/2], sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&d_A1[0], &tmp, sizeof(double), cudaMemcpyHostToDevice);
  }
