...
__global__ void matrix_multiply(float *A, float *B, float *C, size_t N)
{
  ...
  int ntx=16, nty = 16;
  dim2 threads(ntx,nty,1);
  dim2 grid(N/threads.x.N /threads.y);

  __shared__ float ATile[TILE_DIM] [TILE_DIM] //FIXME//fixed
  ...
  for (int p = 0; p < N/TILE_DIM; ++p)
  {
    ...
    BTile[ty][tx] = B[p*TILE_DIM +ty)*N + col]; // FIXME/fixed
...
int main(void)
{
  ...
  float optimized_throughput = static_cast<float>(2 * n * n * n) / (average_optimized_time / 1000.0f) / 100000000                  0.0f;
